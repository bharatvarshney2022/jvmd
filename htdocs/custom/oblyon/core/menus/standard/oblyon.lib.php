 ...
 * @return	void
 */
function print_oblyon_menu_layout($db,$atarget,$type_user,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='',$moredata=null) {
    ...
    if (empty($noout)) print '							<ul class="menu-nav">';//print_start_menu_array();
	...
	if ($showmode)
	{
		...
		else $classname = 'class="menu-section tmenu"';
		$idsel='members';
	...
	if (empty($noout)) print_end_menu_array_layout();
...
function print_oblyon_menu($db,$atarget,$type_user,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='',$moredata=null) {
	...
	if (empty($menu_invert) && empty($noout) && ! empty($conf->global->MAIN_SHOW_LOGO)) {
        ...
        print "\n".'<!-- Show logo on menu -->'."\n";
        print_start_menu_entry('companylogo', 'class="menu-section tmenu tmenucompanylogo"', 1);
...
function print_end_menu_entry_layout($showmode)
{
	if ($showmode)
	{
		print "\n".'								</li>';
...
function print_end_menu_array_layout() {
	print '							</ul>';
	print "\n";
}
 ...
 * @return	void
 */
function print_left_oblyon_menu_layout($db,$menu_array_before,$menu_array_after,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='',$moredata=null)
{
	global $user,$conf,$langs,$dolibarr_main_db_name,$mysoc;
		...
		if ($mainmenu == 'companies')
		{
			...
			if (! empty($conf->societe->enabled))
			{
				...
				if ($user->rights->societe->creer)
				{
					...
					///$newmenu->add("/societe/card.php?action=create&type=c", $langs->trans("MenuNewThirdParty"),1);
	...
	for ($i = 0; $i < $num; $i++) {
		...
		if ($level == 0){
			if ($menu_array[$i]['enabled']) {
				...
				{
					print '												<li class="menu-item menu-item-submenu menu-item-rel" data-menu-toggle="click" aria-haspopup="true">'."\n";	
				}

				print '													<a class="menu-link menu-toggle" href="'.$url.'"'.($menu_array[$i]['target']?' target="'.$menu_array[$i]['target'].'"':'').'>';
					print (!empty($menu_array[$i]['leftmenu'])?'': '').'<span class="menu-text">'.$menu_array[$i]['titre'].' '.(!empty($menu_ivert) && !empty($menu_array[$i+1]['level'])?'<span class="caret	caret--top"></span>':'').'</span><i class="menu-arrow"></i>'; // class="icon icon--'.$menu_array[$i]['leftmenu'].'"></i>
				print '</a>'."\n";
			} else if ($showmenu) {
		...
		// Menu Level = 1 or 2
		echo $level.">".$menu_array[$i]['enabled'].'::';		
