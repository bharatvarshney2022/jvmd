...
llxHeaderLayout('', $langs->trans("Permissions"), $langs->trans("Permissions"));
							...
							<div class="row">
								<div class="col-lg-12">
									...
									<div class="card card-custom gutter-b">
										...
										print dol_get_fiche_head_layout($head, 'rights', $title, -1, 'user');
										...
										if (($caneditperms && empty($objMod->rights_admin_allowed)) || empty($object->admin))
										{
											...
											{
												print '<td class="">';
												print '<a class="" title="'.dol_escape_htmltag($langs->trans("All")).'" alt="'.dol_escape_htmltag($langs->trans("All")).'" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=addrights&amp;entity='.$entity.'&amp;module=allmodules&amp;confirm=yes&amp;token='.newToken().'">'.$langs->trans("All")."</a>";
												print ' / ';
												print '<a class="" title="'.dol_escape_htmltag($langs->trans("None")).'" alt="'.dol_escape_htmltag($langs->trans("None")).'" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=delrights&amp;entity='.$entity.'&amp;module=allmodules&amp;confirm=yes&amp;token='.newToken().'">'.$langs->trans("None")."</a>";
												print '</td>';
											}
											print '<td class="" width="24">&nbsp;</td>';
										}
												...
												if ($result)
												{
													...
													while ($i < $num)
													{
														...
														if (!empty($object->admin) && !empty($objMod->rights_admin_allowed))    // Permission granted because admin
														{
															if ($caneditperms)
															{
																print '<td class="">'.img_picto($langs->trans("Administrator"), 'star').'</td>';
															}
															print '<td class="">';
														...
														} elseif (in_array($obj->id, $permsuser))					// Permission granted by user
														{
															if ($caneditperms)
															{
																print '<td class=""><a class="reposition" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=delrights&amp;entity='.$entity.'&amp;rights='.$obj->id.'&amp;confirm=yes&amp;token='.newToken().'">';
														...
														} elseif (is_array($permsgroupbyentity[$entity]))
														{
															if (in_array($obj->id, $permsgroupbyentity[$entity]))	// Permission granted by group
															{
																...
																if ($caneditperms)
																{
																	print '<td class=""><a class="reposition" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=addrights&amp;entity='.$entity.'&amp;rights='.$obj->id.'&amp;confirm=yes&amp;token='.newToken().'">';
														...
														// Permission id
														if ($user->admin) print '<td class=""><span class="opacitymedium">'.$obj->id.'</span></td>';
