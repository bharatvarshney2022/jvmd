		...
		foreach ($extrafields->attributes[$extrafieldsobjectkey]['label'] as $key => $val)
		{
			if (!empty($arrayfields[$extrafieldsobjectprefix.$key]['checked']))
			{
				...
				if ($extrafields->attributes[$extrafieldsobjectkey]['type'][$key] == 'separate') {
					...
					print getTitleFieldOfListLayout($extrafields->attributes[$extrafieldsobjectkey]['label'][$key], 0, $_SERVER["PHP_SELF"], $sortonfield, "", $param, ($align ? 'align="'.$align.'" data-titlekey="'.$key.'"' : 'data-titlekey="'.$key.'"'), $sortfield, $sortorder, '', $disablesortlink, $tooltip)."\n";
				}
			}
		}
	}
