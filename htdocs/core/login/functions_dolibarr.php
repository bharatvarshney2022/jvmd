...
function check_user_mobile_email($usermobile, $email, $entitytotest = 1)
{
	global $db,$conf,$langs;
	...
	if (! empty($usermobile))
	{
		dol_syslog("functions_subpe::check_user_mobile_email usermobile=".$usermobile." entitytotest=".$entitytotest);
		...
		$sql ='SELECT rowid, fk_soc, firstname, lastname, email, phone_mobile, photo, otp, device_id, statut';
		$sql.=' FROM '.$table;
		$sql.= " WHERE phone_mobile = '".$db->escape($usermobile)."' AND email = '".$db->escape($email)."' ";
		// Required to first found the user into entity, then the superadmin.
		// For the case (TODO and that we must avoid) a user has renamed its login with same value than a user in entity 0.
		$sql.=' ORDER BY entity DESC';
		//echo $sql;
		...
		if ($resql)
		{
			...
			if ($obj)
			{
				// We must check entity
				 $phone_mobile = $obj->phone_mobile;		
				dol_syslog("functions_subpe::check_user_mobile_email Authentification ko user not found for '".$usertotest."'");
		...
		{
			dol_syslog("functions_subpe::check_user_mobile_email Authentification ko db error for '".$usertotest."' error=".$db->lasterror());
		}
	}
...
function check_user_email($email, $entitytotest = 1)
{
	global $db,$conf,$langs;
	...
	if (! empty($email))
	{
		dol_syslog("functions_subpe::check_user_email useremail=".$email." entitytotest=".$entitytotest);
		...
		if ($resql)
		{
			...
			if ($obj)
			{
				...
				dol_syslog("functions_subpe::check_user_email Authentification ko user not found for '".$usertotest."'");
		...
		{
			dol_syslog("functions_subpe::check_user_email Authentification ko db error for '".$usertotest."' error=".$db->lasterror());
		}
	}
...
function check_user_device($device_id, $entitytotest = 1)
{
	global $db,$conf,$langs;
	...
	if (! empty($device_id))
	{
		...
		$sql ='SELECT rowid, fk_soc, firstname, lastname, email, phone_mobile, photo, otp, device_id, statut';
		$sql.=' FROM '.$table;
		$sql.= " WHERE device_id = '".$db->escape($device_id)."' ";
		// Required to first found the user into entity, then the superadmin.
		// For the case (TODO and that we must avoid) a user has renamed its login with same value than a user in entity 0.
		$sql.=' ORDER BY entity DESC';
		//echo $sql;
		...
		if ($resql)
		{
			...
			if ($obj)
			{
				// We must check entity
				 $phone_mobile = $obj->phone_mobile;		
				dol_syslog("functions_subpe::check_user_device Authentification ko user not found for '".$usertotest."'");
		...
		{
			dol_syslog("functions_subpe::check_user_device Authentification ko db error for '".$usertotest."' error=".$db->lasterror());
		}
	}
