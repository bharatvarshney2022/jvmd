...
class FormCompany extends Form
{
			...
			if ($num)
			{
				...
				while ($i < $num)
				{
					...
					if ($obj->rowid == 0) {
						...
						if ($selected > 0 && $selected == $obj->rowid)
						{
							print '<option value="'.$obj->rowid.'" selected>'.$obj->code.'</option>';
						} else {
							print '<option value="'.$obj->rowid.'">'.$obj->code.'</option>';
						}
					}
					$i++;
				}
	...
	public function select_brand($selected = '', $country_codeid = 0, $htmlname = 'brand')
	{
		...
		if ($resql)
		{
			print '<select class="brand form-control" id="'.$htmlname.'" name="'.$htmlname.'">';
