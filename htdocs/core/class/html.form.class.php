...
class Form
{
	...
	public function editfieldkey($text, $htmlname, $preselected, $object, $perm, $typeofdata = 'string', $moreparam = '', $fieldrequired = 0, $notabletag = 0, $paramid = 'id', $help = '')
	{
		...
		if (!empty($conf->global->MAIN_USE_JQUERY_JEDITABLE) && !preg_match('/^select;/', $typeofdata))
		{
			...
			if (empty($notabletag) && GETPOST('action', 'aZ09') != 'edit'.$htmlname && $perm) $ret .= '<table class="table table-borderless"><tr><td class="">';
			if ($fieldrequired) $ret .= '<span class="fieldrequired">';
			if ($help) {
				$ret .= $this->textwithpicto($langs->trans($text), $help);
			} else {
			...
			if (empty($notabletag) && GETPOST('action', 'aZ09') != 'edit'.$htmlname && $perm) $ret .= '<td class="">';
	...
	public function editfieldval($text, $htmlname, $value, $object, $perm, $typeofdata = 'string', $editvalue = '', $extObject = null, $custommsg = null, $moreparam = '', $notabletag = 0, $formatfunc = '', $paramid = 'id')
	{
		...
		if (!empty($conf->global->MAIN_USE_JQUERY_JEDITABLE) && !preg_match('/^select;|datehourpicker/', $typeofdata)) // TODO add jquery timepicker and support select
		{
			...
			if ($editmode)
			{
				...
				$ret .= '<input type="submit" class="btn btn-info'.(empty($notabletag) ? '' : ' ').'" name="modify" value="'.$langs->trans("Modify").'">';
				if (preg_match('/ckeditor|textarea/', $typeofdata) && empty($notabletag)) $ret .= '<br>'."\n";
				$ret .= '<input type="submit" class="btn btn-danger button-cancel'.(empty($notabletag) ? '' : ' ').'" name="cancel" value="'.$langs->trans("Cancel").'">';
				if (empty($notabletag)) $ret .= '</td>';
	...
	public function selectMassActionLayout($selected, $arrayofaction, $alwaysvisible = 0, $name = 'massaction', $cssclass = 'checkforselect')
	{
		...
		$disabled = 0;
		$ret .= '<select class="form-control'.(empty($conf->use_javascript_ajax) ? '' : ' hideobject').' '.$name.' '.$name.'select valignmiddle alignstart" id="'.$name.'" name="'.$name.'"'.($disabled ? ' disabled="disabled"' : '').'>';
	...
	public function select_incoterms($selected = '', $location_incoterms = '', $page = '', $htmlname = 'incoterm_id', $htmloption = '', $forcecombo = 1, $events = array())
	{
		...
		if ($resql)
		{
			...
			$out .= '<select id="'.$htmlname.'" class="form-control selectincoterm minwidth100imp noenlargeonsmartphone" name="'.$htmlname.'" '.$htmloption.'>';
	...
	public function select_type_of_lines($selected = '', $htmlname = 'type', $showempty = 0, $hidetext = 0, $forceall = 0)
	{
			...
			|| (empty($forceall) && empty($conf->product->enabled) && empty($conf->service->enabled)))
		{
			...
			print '<select class="form-control" id="select_'.$htmlname.'" name="'.$htmlname.'">';
			if ($showempty)
			{
	...
	public function select_type_fees($selected = '', $htmlname = 'type', $showempty = 0)
	{
		...
		print '<select id="select_'.$htmlname.'" class="form-control" name="'.$htmlname.'">';
		if ($showempty)
		{
			print '<option value="-1"';
			if ($selected == -1) print ' selected';
	 ...
	 */
	public function select_company($selected = '', $htmlname = 'socid', $filter = '', $showempty = '', $showtype = 0, $forcecombo = 0, $events = array(), $limit = 0, $morecss = '', $moreparam = '', $selected_input_value = '', $hidelabel = 1, $ajaxoptions = array(), $multiple = false)
	{
		// phpcs:enable
	 ...
	 */
	public function select_thirdparty_list($selected = '', $htmlname = 'socid', $filter = '', $showempty = '', $showtype = 0, $forcecombo = 0, $events = array(), $filterkey = '', $outputmode = 0, $limit = 0, $morecss = '', $moreparam = '', $multiple = false)
	{
		// phpcs:enable
		global $conf, $user, $langs;
		...
		if ($resql)
		{
			...
			$out .= '<select id="'.$htmlname.'" class="form-control'.($morecss ? ' '.$morecss : '').'"'.($moreparam ? ' '.$moreparam : '').' name="'.$htmlname.($multiple ? '[]' : '').'" '.($multiple ? 'multiple' : '').'>'."\n";
	...
	public function select_remises($selected, $htmlname, $filter, $socid, $maxvalue = 0)
	{
		...
		if ($resql)
		{
			print '<select id="select_'.$htmlname.'" class="form-control maxwidthonsmartphone" name="'.$htmlname.'">';
			$num = $this->db->num_rows($resql);
	...
	public function selectcontacts($socid, $selected = '', $htmlname = 'contactid', $showempty = 0, $exclude = '', $limitto = '', $showfunction = 0, $moreclass = '', $options_only = false, $showsoc = 0, $forcecombo = 0, $events = array(), $moreparam = '', $htmlid = '', $multiple = false, $disableifempty = 0)
	{
		...
		if ($resql)
		{
			...
			if ($htmlname != 'none' && !$options_only) {
				$out .= '<select class="form-control'.($moreclass ? ' '.$moreclass : '').'" id="'.$htmlid.'" name="'.$htmlname.(($num || empty($disableifempty)) ? '' : ' disabled').($multiple ? '[]' : '').'" '.($multiple ? 'multiple' : '').' '.(!empty($moreparam) ? $moreparam : '').'>';
			}
	...
	public function select_dolusers($selected = '', $htmlname = 'userid', $show_empty = 0, $exclude = null, $disabled = 0, $include = '', $enableonly = '', $force_entity = '0', $maxlength = 0, $showstatus = 0, $morefilter = '', $show_every = 0, $enableonlytext = '', $morecss = '', $noactive = 0, $outputmode = 0, $multiple = false)
	{
		...
		if ($resql)
		{
			...
			if ($num)
			{
				...
				$out .= '<select class="form-control'.($morecss ? ' '.$morecss : ' ').'" id="'.$htmlname.'" name="'.$htmlname.($multiple ? '[]' : '').'" '.($multiple ? 'multiple' : '').' '.($disabled ? ' disabled' : '').'>';
				...
				$out .= '<select class="form-control" id="'.$htmlname.'" name="'.$htmlname.'" disabled>';
	...
	public function select_produits($selected = '', $htmlname = 'productid', $filtertype = '', $limit = 0, $price_level = 0, $status = 1, $finished = 2, $selected_input_value = '', $hidelabel = 0, $ajaxoptions = array(), $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $hidepriceinlabel = 0, $warehouseStatus = '', $selected_combinations = array(), $nooutput = 0)
	{
		...
		if (!empty($conf->use_javascript_ajax) && !empty($conf->global->PRODUIT_USE_SEARCH_TO_SELECT))
		{
			...
			$out .= '<input type="text" class="form-control" name="search_'.$htmlname.'" id="search_'.$htmlname.'" value="'.$selected_input_value.'"'.$placeholder.' '.(!empty($conf->global->PRODUCT_SEARCH_AUTOFOCUS) ? 'autofocus' : '').' />';
			if ($hidelabel == 3) {
				$out .= img_picto($langs->trans("Search"), 'search');
			}
		} else {
	...
	public function select_produits_fournisseurs_list($socid, $selected = '', $htmlname = 'productid', $filtertype = '', $filtre = '', $filterkey = '', $statut = -1, $outputmode = 0, $limit = 100, $alsoproductwithnosupplierprice = 0, $morecss = '', $showstockinlist = 0)
	{
		...
		if ($result)
		{
			...
			$out .= '<select class="form-control maxwidthonsmartphone'.($morecss ? ' '.$morecss : '').'" id="'.$htmlname.'" name="'.$htmlname.'">';
	...
	public function select_product_fourn_price($productid, $htmlname = 'productfournpriceid', $selected_supplier = '')
	{
		...
		if ($result)
		{
			...
			$form = '<select class="form-control" id="select_'.$htmlname.'" name="'.$htmlname.'">';
	...
	public function selectAvailabilityDelay($selected = '', $htmlname = 'availid', $filtertype = '', $addempty = 0)
	{
		...
		print '<select id="'.$htmlname.'" class="form-control" name="'.$htmlname.'">';
	...
	public function select_conditions_paiements($selected = 0, $htmlname = 'condid', $filtertype = -1, $addempty = 0, $noinfoadmin = 0, $morecss = '')
	{
		...
		print '<select id="'.$htmlname.'" class="form-control selectpaymentterms'.($morecss ? ' '.$morecss : '').'" name="'.$htmlname.'">';
		if ($addempty) print '<option value="0">&nbsp;</option>';
	...
	public function select_types_paiements($selected = '', $htmlname = 'paiementtype', $filtertype = '', $format = 0, $empty = 1, $noadmininfo = 0, $maxlength = 0, $active = 1, $morecss = '')
	{
		...
		print '<select id="select'.$htmlname.'" class="form-control selectpaymenttypes'.($morecss ? ' '.$morecss : '').'" name="'.$htmlname.'">';
	...
	public function selectPriceBaseType($selected = '', $htmlname = 'price_base_type', $addjscombo = 0)
	{
		...
		$return = '<select class="form-control maxwidth100" id="select_'.$htmlname.'" name="'.$htmlname.'">';
		$options = array(
			'HT'=>$langs->trans("HT"),
			'TTC'=>$langs->trans("TTC")
		);
	...
	public function selectTransportMode($selected = '', $htmlname = 'transportmode', $format = 0, $empty = 1, $noadmininfo = 0, $maxlength = 0, $active = 1, $morecss = '')
	{
		...
		print '<select id="select'.$htmlname.'" class="form-control selectmodetransport'.($morecss ? ' '.$morecss : '').'" name="'.$htmlname.'">';
		if ($empty) print '<option value="">&nbsp;</option>';
	...
	public function selectShippingMethod($selected = '', $htmlname = 'shipping_method_id', $filtre = '', $useempty = 0, $moreattrib = '')
	{
		...
		if ($result) {
			...
			$i = 0;
			if ($num) {
				print '<select id="select'.$htmlname.'" class="form-control selectshippingmethod" name="'.$htmlname.'"'.($moreattrib ? ' '.$moreattrib : '').'>';
	...
	public function selectUnits($selected = '', $htmlname = 'units', $showempty = 0)
	{
		...
		$return = '<select class="form-control" id="'.$htmlname.'" name="'.$htmlname.'">';
	...
	public function select_comptes($selected = '', $htmlname = 'accountid', $status = 0, $filtre = '', $useempty = 0, $moreattrib = '', $showcurrency = 0, $morecss = '', $nooutput = 0)
	{
		...
		if ($result)
		{
			...
			if ($num)
			{
				$out .= '<select id="select'.$htmlname.'" class="form-control selectbankaccount'.($morecss ? ' '.$morecss : '').'" name="'.$htmlname.'"'.($moreattrib ? ' '.$moreattrib : '').'>';
				if ($useempty == 1 || ($useempty == 2 && $num > 1))
				{
	...
	public function selectEstablishments($selected = '', $htmlname = 'entity', $status = 0, $filtre = '', $useempty = 0, $moreattrib = '')
	{
		...
		if ($result)
		{
			...
			if ($num)
			{
				print '<select id="select'.$htmlname.'" class="form-control selectestablishment" name="'.$htmlname.'"'.($moreattrib ? ' '.$moreattrib : '').'>';
				if ($useempty == 1 || ($useempty == 2 && $num > 1))
				{
					print '<option value="-1">&nbsp;</option>';
				}
		...
		if (is_array($formquestion) && !empty($formquestion))
		{
			...
			foreach ($formquestion as $key => $input)
			{
				if (is_array($input) && !empty($input))
				{
					...
					if ($input['type'] == 'text')
					{
						$more .= '<div class="tagtr"><div class="tagtd'.(empty($input['tdclass']) ? '' : (' '.$input['tdclass'])).'">'.$input['label'].'</div><div class="tagtd"><input type="text" class="form-control'.$morecss.'" id="'.$input['name'].'" name="'.$input['name'].'"'.$size.' value="'.$input['value'].'"'.$moreattr.' /></div></div>'."\n";
					} elseif ($input['type'] == 'password')
					{
						$more .= '<div class="tagtr"><div class="tagtd'.(empty($input['tdclass']) ? '' : (' '.$input['tdclass'])).'">'.$input['label'].'</div><div class="tagtd"><input type="password" class="form-control'.$morecss.'" id="'.$input['name'].'" name="'.$input['name'].'"'.$size.' value="'.$input['value'].'"'.$moreattr.' /></div></div>'."\n";
					...
					} elseif ($input['type'] == 'checkbox')
					{
						...
						$more .= '<input type="checkbox" class="form-control'.$morecss.'" id="'.$input['name'].'" name="'.$input['name'].'"'.$moreattr;
					...
					} elseif ($input['type'] == 'radio')
					{
						...
						foreach ($input['values'] as $selkey => $selval)
						{
							...
							$more .= '<div class="tagtd'.($i == 0 ? ' tdtop' : '').'"><input type="radio" class="form-control'.$morecss.'" id="'.$input['name'].$selkey.'" name="'.$input['name'].'" value="'.$selkey.'"'.$moreattr;
					...
					} elseif ($input['type'] == 'datetime')
					{
						$more .= '<div class="tagtr"><div class="tagtd'.(empty($input['tdclass']) ? '' : (' '.$input['tdclass'])).'">'.$input['label'].'</div>';
						$more .= '<div class="tagtd">';
						$more .= $this->selectDate($input['value'], $input['name'], 2, 1, 2, 'timespent_date', 1, 0);
						$more .= '</div></div>'."\n";
						$formquestion[] = array('name'=>$input['name'].'day');
						$formquestion[] = array('name'=>$input['name'].'month');
						$formquestion[] = array('name'=>$input['name'].'year');
	...
	public function selectCurrency($selected = '', $htmlname = 'currency_id', $mode = 0)
	{
		...
		$out .= '<select class="form-control" name="'.$htmlname.'" id="'.$htmlname.'" style="width:100%">';
	...
	public function selectDate($set_time = '', $prefix = 're', $h = 0, $m = 0, $empty = 0, $form_name = "", $d = 1, $addnowlink = 0, $disabled = 0, $fullday = '', $addplusone = '', $adddateof = '', $openinghours = '', $stepminutes = 1, $labeladddateof = '', $placeholder = '', $gm = 'auto')
	{
		...
		if ($d)
		{
			...
			// Show date with combo selects
				...
				if ($emptydate || $set_time == -1)
				{
					$retstring .= '<input'.($disabled ? ' disabled' : '').' placeholder="'.dol_escape_htmltag($langs->trans("Year")).'" class="form-control maxwidth50" type="number" min="0" max="3000" maxlength="4" id="'.$prefix.'year" name="'.$prefix.'year" value="'.$syear.'">';
				} else {
					$retstring .= '<select'.($disabled ? ' disabled' : '').' class="form-control maxwidth75imp" id="'.$prefix.'year" name="'.$prefix.'year">';
	 ...
	 */
	public static function multiselectarraywithdisable($htmlname, $array, $selected = array(), $key_in_label = 0, $value_as_key = 0, $morecss = '', $disbaledarray = array(), $translate = 0, $width = 0, $moreattrib = '', $elemtype = '', $placeholder = '', $addjscombo = -1)
	{
		global $conf, $langs;
	...
	public static function multiselectarray($htmlname, $array, $selected = array(), $key_in_label = 0, $value_as_key = 0, $morecss = '', $translate = 0, $width = 0, $moreattrib = '', $elemtype = '', $placeholder = '', $addjscombo = -1)
	{
		...
		// Try also magic suggest
		$out .= '<select id="'.$htmlname.'" class="form-control multiselect'.($morecss ? ' '.$morecss : '').'" multiple name="'.$htmlname.'[]"'.($moreattrib ? ' '.$moreattrib : '').($width ? ' style="width: '.(preg_match('/%/', $width) ? $width : $width.'px').'"' : '').'>'."\n";
		if (is_array($array) && !empty($array))
		{
			if ($value_as_key) $array = array_combine($array, $array);
	...
	public function selectyesno($htmlname, $value = '', $option = 0, $disabled = false, $useempty = 0, $addjscombo = 0)
	{
		...
		$resultyesno = '<select class="form-control" id="'.$htmlname.'" name="'.$htmlname.'"'.$disabled.'>'."\n";
	...
	public function showFilterButtons()
	{
		...
		$out .= '<button type="submit" class="btn btn-info button_search" name="button_search_x" value="x"><span class="fa fa-search"></span></button>';
		$out .= '&nbsp;&nbsp; <button type="submit" class="btn btn-warning button_removefilter" name="button_removefilter_x" value="x"><span class="fa fa-remove"></span></button>';
		$out .= '</div>';
