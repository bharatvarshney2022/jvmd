...
function show_contacts($conf, $langs, $db, $object, $backtopage = '')
{
	...
	print $hookmanager->resPrint;
	print getTitleFieldOfList($selectedfields, 0, $_SERVER["PHP_SELF"], '', '', '', 'align="center"', $sortfield, $sortorder, ' ')."\n";
	print '</tr>'."\n";
...
function show_contacts_layout($conf, $langs, $db, $object, $backtopage = '')
{
	global $user, $conf, $extrafields, $hookmanager;
	global $contextpage;
	...
	foreach ($contactstatic->fields as $key => $val) {
		...
		if ($key == 'status' || $key == 'statut') $align .= ($align ? ' ' : '').'center';
		if (!empty($arrayfields['t.'.$key]['checked']) || !empty($arrayfields['sc.'.$key]['checked'])) {
			print '<td class="'.($align ? ' '.$align : '').'">';
			if (in_array($key, array('statut'))) {
				print $form->selectarray('search_status', array('-1'=>'', '0'=>$contactstatic->LibStatut(0, 1), '1'=>$contactstatic->LibStatut(1, 1)), $search_status);
			} elseif (in_array($key, array('role'))) {
				print $formcompany->showRoles("search_roles", $contactstatic, 'edit', $search_roles);
			} else {
				print '<input type="text" class="form-control" name="search_'.$key.'" value="'.dol_escape_htmltag($search[$key]).'">';
			}
			print '</td>';
		}
	}
	...
	foreach ($contactstatic->fields as $key => $val) {
		$align = '';
		if (in_array($val['type'], array('date', 'datetime', 'timestamp'))) $align .= ($align ? ' ' : '').'';
		if (in_array($val['type'], array('timestamp'))) $align .= ($align ? ' ' : '').'';
		if ($key == 'status' || $key == 'statut') $align .= ($align ? ' ' : '').'';
		if (!empty($arrayfields['t.'.$key]['checked'])) print getTitleFieldOfListLayout($val['label'], 0, $_SERVER['PHP_SELF'], 't.'.$key, '', $param, ($align ? 'class="'.$align.'"' : ''), $sortfield, $sortorder, $align.' ')."\n";
		if ($key == 'role') $align .= ($align ? ' ' : '').'';
		if (!empty($arrayfields['sc.'.$key]['checked'])) {
			print getTitleFieldOfListLayout($arrayfields['sc.'.$key]['label'], 0, $_SERVER['PHP_SELF'], '', '', $param, ($align ? 'class="'.$align.'"' : ''), $sortfield, $sortorder, $align.' ')."\n";
		}
	}
	// Extra fields
	$extrafieldsobjectkey = $contactstatic->table_element;
	...
	$reshook = $hookmanager->executeHooks('printFieldListTitle', $parameters, $object); // Note that $action and $object may have been modified by hook
	print $hookmanager->resPrint;
	print getTitleFieldOfListLayout($selectedfields, 0, $_SERVER["PHP_SELF"], '', '', '', 'align=""', $sortfield, $sortorder, ' ')."\n";
	print '</tr></thead>'."\n";
	...
	if ($num || (GETPOST('button_search') || GETPOST('button_search.x') || GETPOST('button_search_x'))) {
		...
		while ($i < $num) {
			...
			// Edit
			if ($user->rights->societe->contact->creer) {
				print '<a class="btn btn-info" href="'.DOL_URL_ROOT.'/contact/card.php?action=edit&id='.$obj->rowid.'&backtopage='.urlencode($backtopage).'">';
				print img_edit();
				print '</a>';
			}
...
function show_actions_done($conf, $langs, $db, $filterobj, $objcon = '', $noprint = 0, $actioncode = '', $donetodo = 'done', $filters = array(), $sortfield = 'a.datep,a.id', $sortorder = 'DESC', $module = '')
{
	...
	if (!empty($conf->agenda->enabled) || (!empty($conf->mailing->enabled) && !empty($objcon->email))) {
		...
		$out .= getTitleFieldOfList('', 0, $_SERVER["PHP_SELF"], '', '', $param, '', $sortfield, $sortorder, ' ');
		$out .= '</tr>';
