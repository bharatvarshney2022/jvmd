	...
	if($userExists)
	{
		...
		if ($result) {
			...
			while ($i < $num) {
				...
				$societeLeadData[] = array('lead_id' => $obj->id, 'lead_code' => $obj->ref, 'status' => $leadStatus, 'call_source' => $obj->call_source, 'service_type' => $obj->service_type, 'brand' => $obj->brand_name, 'category_name' => $obj->category_name, 'sub_category_name' => $obj->sub_category_name, 'model' => $obj->model_name, 'product_name' => $obj->product_name, 'date_added' => date('b d M Y h:i A', strtotime($obj->date_creation)));
				$i++;
			}
